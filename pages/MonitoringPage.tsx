...
import { AttendanceRecord, Teacher, ClassRoom, Student, Headmaster, AttendanceStatus, ViolationRecord, ViolationItem } from '../types';
...
const MonitoringPage: React.FC<MonitoringPageProps> = ({ 
  records, teachers, classes, students, violationRecords, violationItems, headmaster, setHeadmaster, onLogout 
}) => {
  ...
  // 1. Monitor Guru Mengajar Today
  const todayTeachers = teachers.map(t => {
    const teachingToday = records.filter(r => r.teacherId === t.id && r.date === selectedDate);
    return {
      ...t,
      sessions: teachingToday.length,
      lastSession: teachingToday[teachingToday.length-1]?.createdAt || '-'
    };
    ...
    <AdminLayout onLogout={onLogout}>
        ...
        <div className="lg:col-span-3 space-y-8">
          <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="p-6 border-b border-slate-200 bg-slate-50/50 flex justify-between items-center">
              ...
              <span className="text-[10px] font-black px-3 py-1 bg-emerald-100 text-emerald-700 rounded-full">{todayTeachers.length} GURU AKTIF</span>
            </div>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-white border-b border-slate-100 text-slate-400 text-[10px] font-black uppercase tracking-widest">
                    ...
                    <th className="px-6 py-4 text-center">Absen Terakhir</th>
                  </tr>
                </thead>
                  ...
                  {todayTeachers.map(t => (
                    <tr key={t.id} className="hover:bg-blue-50/30 transition-colors">
                      ...
                      <td className="px-6 py-4 text-center">
                        <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-xs font-black">{t.sessions} Sesi</span>
                      </td>
                      <td className="px-6 py-4 text-center font-mono text-xs text-slate-500">{t.lastSession}</td>
                    </tr>
                  ))}
                  {todayTeachers.length === 0 && (
                    <tr>
                      <td colSpan={3} className="px-6 py-12 text-center text-slate-300 italic font-bold">Tidak ada aktivitas pada tanggal ini.</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            {/* Tabel Rekap Pelanggaran per Kelas */}
            <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
      ...
      <div className="mt-8 bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-6 bg-rose-50 border-b border-rose-100 flex justify-between items-center">
          ...
          <span className="text-[10px] font-black px-3 py-1 bg-white text-rose-700 rounded-full border border-rose-200">
